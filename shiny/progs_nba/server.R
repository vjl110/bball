  ...
  output$plot1 <- renderPlot({
	...
	abline(h = mean(dat[, as.numeric(input$stat)], na.rm=T), col = "grey")
		...
		for(i in c(unique(set$Exp[set$Name == input$plr]))){
			lws <- loess(set[set$Exp == i & set$Name == input$plr, 2] ~ set$Age[set$Exp == i & set$Name == input$plr], span = as.numeric(input$spn))	
			lines(set$Age[set$Exp == i & set$Name == input$plr], predict(lws, newdat = set[set$Exp == i & set$Name == input$plr, ]), col = "red", lwd = 2)
