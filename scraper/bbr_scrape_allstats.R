...
PBP <- data.frame("Season"=NA, "Age"=NA, "Tm"=NA, "Lg"=NA, "Pos"=NA, "G"=NA, "MP"=NA, "PG."=NA, "SG."=NA, "SF."=NA, "PF."=NA, "C."=NA, "On"=NA, "OnOff"=NA, "BadPass"=NA, "LostBall"=NA, "Other"=NA, "Shoot"=NA, "Block"=NA, "Offens"=NA, "Take"=NA, "PGA"=NA, "Drawn"=NA, "And1"=NA, "Blkd"=NA, "id"=NA)
...
for(i in 1:nrow(IDs)){
	...
	if(length(stat$shooting) > 0){
		...
		colnames(pbp) <- c("Season", "Age", "Tm", "Lg", "Pos", "G", "MP", "PG.", "SG.", "SF.", "PF.", "C.", "On", "OnOff", "BadPass", "LostBall", "Other", "Shoot", "Block", "Offens", "Take", "PGA", "Drawn", "And1", "Blkd")
		pbp$id <- IDs$id[i]
			PBP <- rbind(PBP, pbp)
	}else{
	}
	...
	if(length(stat$playoffs_shooting) > 0){	
		...
		colnames(pbp_po) <- c("Season", "Age", "Tm", "Lg", "Pos", "G", "MP", "PG.", "SG.", "SF.", "PF.", "C.", "On", "OnOff", "BadPass", "LostBall", "Other", "Shoot", "Block", "Offens", "Take", "PGA", "Drawn", "And1", "Blkd")
		pbp_po$id <- IDs$id[i]
...
write.csv(PBP, "data/PBP_pull.csv", row.names = F)
...
write.csv(PBP_PO, "data/PBPPO_pull.csv", row.names = F)
