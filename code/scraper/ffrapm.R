...
OvD.DMP <- list()
=======
	lnp <- read.csv("LUP_RPM.csv", stringsAsFactors=FALSE)
		lnp$EFG.dif <- lnp$EFG_O - lnp$EFG_D
		lnp$TOV.dif <- lnp$TOV_O - lnp$TOV_D
		lnp$REB.dif <- lnp$REB_O - (1 - lnp$REB_D)
		lnp$FT.dif <- lnp$FT_O - lnp$FT_D			
...
=======
	...
	x <- data.matrix(lnp.yr[c(6,30:ncol(lnp.yr))]) #turn the data frame (which is now just 1s, -1s, and 0s) into a matrix
	DVs <- data.frame("EFG_dif" = lnp.yr$EFG.dif, "REB_dif" = lnp.yr$REB.dif, "TOV_dif" = lnp.yr$TOV.dif, "FT_dif" = lnp.yr$FT.dif)
	WGTs <- data.frame("FGA" = lnp.yr$FGA_O + lnp.yr$FGA_D, "RB.chance" = lnp.yr$RB.chance_O + lnp.yr$RB.chance_D, "POSS.to" = lnp.yr$POSS.to_O + lnp.yr$POSS.to_D, "FGA" = lnp.yr$FGA_O + lnp.yr$FGA_D) 
	OUTPUTS <- list()
	for(j in 1:ncol(DVs)){
		...
		ridge <- glmnet(x, DVs[,j], family=c("gaussian"), WGTs[,j], alpha=0.5, lambda=lambda.min) #
>>>>>>> scraping finished working on rapm
...
<<<<<<< HEAD
	...
	OvD <- merge(OvD, OUTPUTS[[4]], by = c("ID", "Season", "Name.full"))	
=======
	dd <- merge(OUTPUTS[[1]], OUTPUTS[[2]], by = c("ID", "Season", "Name.full"))	
	dd <- merge(dd, OUTPUTS[[3]], by = c("ID", "Season", "Name.full"))	
	dd <- merge(dd, OUTPUTS[[4]], by = c("ID", "Season", "Name.full"))	
...
=======
	dd <- merge(dd, mrgset, by = c("ID", "Season", "Name.full"))

	dd <- dd[order(dd$EFG_dif, decreasing =T), ]
